<template>
  <section class="pagination">
    <span class="total">A total of {{ total }}</span>
  ...
  computed: {
    pages () {
      if (this.pageSize > 0) {
        ...
        let that = this
        that.totalPage = pageCount > 0 ? pageCount : 1
        that.currentPage = this.currentPage < 0 ? 1 : this.currentPage
        that.currentPage = this.currentPage > this.totalPage ? this.totalPage : this.currentPage
      }
      let array = []
